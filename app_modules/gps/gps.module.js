...
gps.init = function(m) {
  ...
  var montlyJob = scheduler.scheduleJob("0 0 2 * * *", function() {
    ...
    var query = {
      createdAt: { $lt: new Date(new Date() - 7 * 24 * 60 * 60 * 1000) }
    };

    var m = { pl: { user: { oID: globalConfig.orgs[0].oID } } };

    gps.getAllCars(m).then(function(allCars) {
      var excludeIds = [];
      if (allCars.pl && allCars.pl.cars && allCars.pl.cars.length) {
        excludeIds = allCars.pl.cars.map(function(o) {
          return o.id + "";
        });

        query["_id"] = { $nin: excludeIds };

        gpsModel //get last old request
          .remove(query, function(err, docs) {
            if (!err) {
              console.log("error, data---->>---", docs.result);
            }
          });
      }
    });
...
gps.shipmentComplete = function(m) {
  ...
  if (shipment && shipment.sim && user && user.oID) {
    ...
    Shiment.findOneAndUpdate(
      { _id: shipment._id, oID: user.oID },
      shipment,
      { new: true },
      function(err, resp) {
        if (err) {
