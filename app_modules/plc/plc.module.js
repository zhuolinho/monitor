              ...
              _extractPlcData(
                ...
                if (
                  !(
                    ...
                    dataToSave.dct == "NaN" ||
                    dataToSave.dct == "2" ||
                    dataToSave.cdct == "0-0-0 0:0:0" ||
                    dataToSave.cdct == "1970-1-1 0:0:0" ||
                    dataToSave.cdct == NaN
                  )
                ) {
                  var chanelCheck = _checkChanelInterruption(
                    dataToSave,
                    m.pl.org.oID,
                    latestIncommingData,
                    latestInteruptedChanels
                  );

                  m.redisClient.set(
                    "lastestPlc",
                    JSON.stringify(chanelCheck.latestIncommingData)
                  ); //save updated data
                  m.redisClient.set(
                    "lastestInterupts",
                    JSON.stringify(chanelCheck.latestInteruptedChanels)
                  ); //save updated data

                  if (resp.alert) {
                    //TODO the cng alerts are not being added. should first uncomment the conresponding section on extractCngData function after formula confirmation.
...
plc.getLatestData = function(m) {
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    iPlc
      .find({
        oID: m.pl.user.oID,
        cd: { $gt: lib.deductTimeFromDate(plcConfig.sTimer + 2 * 60 * 1000) }
      })
      .sort({ cd: -1 })
      .limit(length)
    ...
    iPlc
      ...
      .exec(function(err, plc) {
        if (err) {
          ...
          r.pl.plc = {
            values: instantaneousValues1,
            values2: instantaneousValues2,
            dates: dates
          };
          r.status = true;
          deferred.resolve(r);
        ...
        iPlc
          ...
          .exec(function(err, plc) {
            ...
            if (err) {
              ...
              if (plc && plc.length) {
                var previousDate =
                  lib.getPreviousMonthDate(m.pl.start) + " 00:00:00"; // shift back one month
                iPlc
                  .find({
                    oID: m.pl.user.oID,
                    cd: { $gte: previousDate, $lt: start }
                  })
                  .sort({ cd: -1 })
                  .limit(1)
                  .exec(function(err2, plc2) {
                    ...
                    if (err2) {
                      ...
                      if (plc2.length) {
                        plc[0].usage1 = Math.abs(
                          parseInt(plc[0].maxVal1 - plc2[0][flow1], 10)
                        );
                        plc[0].usage2 = Math.abs(
                          parseInt(plc[0].maxVal2 - plc2[0][flow2], 10)
                        );
                      } else {
                      ...
                      for (var i = 1; i < plc.length; i++) {
                        plc[i].usage1 = Math.abs(
                          parseInt(plc[i].maxVal1 - plc[i - 1].maxVal1, 10)
                        );
                        plc[i].usage2 = Math.abs(
                          parseInt(plc[i].maxVal2 - plc[i - 1].maxVal2, 10)
                        );
          ...
          .aggregate([
            {
              $match: {
                oID: m.pl.user.oID,
                tank: m.pl.tank,
                cd: { $gte: startMonth, $lte: endMonth }
              }
            },
            {
              $group: {
                ...
                iPlc
                  ...
                  .exec(function(err2, plc2) {
                    ...
                    if (err2) {
                      ...
                      if (plc2.length) {
                        plc[0].usage1 = Math.abs(
                          parseFloat(plc[0].maxVal1 - plc2[0][flow1])
                        );
                        plc[0].usage2 = Math.abs(
                          parseFloat(plc[0].maxVal2 - plc2[0][flow2])
                        );
                      } else {
                        // plc[0].usage  =  plc[0].maxVal;
                        plc[0].usage1 = null;
                      ...
                      for (var i = 1; i < plc.length; i++) {
                        plc[i].usage1 = Math.abs(
                          parseFloat(plc[i].maxVal1 - plc[i - 1].maxVal1)
                        );
                        plc[i].usage2 = Math.abs(
                          parseFloat(plc[i].maxVal2 - plc[i - 1].maxVal2)
                        );
...
plc.getAddress = function(m) {
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    Address.find({ oID: m.pl.user.oID })
      .sort({ cd: -1 })
...
plc.getFormula = function(m) {
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    PlcFormula.find({ oID: m.pl.user.oID })
      .sort({ cd: -1 })
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    if (m.pl && m.pl.address) {
      Address.findOneAndUpdate(
        { _id: m.pl.address._id, oID: m.pl.user.oID },
        m.pl.address,
        { new: true },
...
plc.updateFormula = function(m) {
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    if (m.pl && m.pl.formula) {
      PlcFormula.findOneAndUpdate(
        { _id: m.pl.formula._id, oID: m.pl.user.oID },
        m.pl.formula,
        { new: true },
        function(err, resp) {
          if (err) {
            ...
            m.redisClient.get("lastestFormula", function(err, result) {
              //all latest formula
              var temp = {};
              temp[m.pl.user.oID] = {};
...
plc.updatePlcAlert = function(m) {
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    if (m.pl && m.pl.data && m.pl.data._id) {
      PlcAlert.findOneAndUpdate(
        { _id: m.pl.data._id, oID: m.pl.user.oID },
        m.pl.data,
        { new: true },
        function(err, resp) {
          if (err) {
...
var _extractDates = function(data, shift) {
  ...
  //not needed
  chanelDate =
    parseInt(cyear.toString("hex"), 16) +
    "-" +
  ...
  return {
    date: date,
    chanelDate: chanelDate,
    cnanosecond: parseInt(cnanosecond.toString("hex"), 16)
  };
};
...
var _extractCngData = function(
  ...
  var procesedInfo = _checkCngAlert(
    inputP1,
    inputP2,
    formula,
    cumfow,
...
var _checkCngAlert = function(
  ...
  if (latestIncommingData[oID] && latestIncommingData[oID][tank]) {
    ...
    ); //(Nm3/h)=>Usage for plcConfig.sTimer time interval
  } else {
    usagePerHour = cumfow;
  }
  ...
  if (rft > 0 && rft < formula.tt) {
    // TODO: uncoment this once the condition is clear
    ...
    //TODO remove only when it has been processed
    // if (lastestRemainingAmountAlerts[oID][tank]){  //remove remaining amount alert if previously created to this tank
    //   delete lastestRemainingAmountAlerts[oID][tank];
    //   redisClient.set("lastestRemainingAmountAlerts",JSON.stringify(lastestRemainingAmountAlerts));
    // };
...
var _extractLngData = function(
  ...
  if (remainingAmount < formula.pt) {
    ...
    //TODO remove only after processed
    // if (lastestRemainingAmountAlerts[oID][tank]){  //remove remaining amount alert if previously created to this tank
    //   delete lastestRemainingAmountAlerts[oID][tank];
    //   redisClient.set("lastestRemainingAmountAlerts",JSON.stringify(lastestRemainingAmountAlerts));
    // };
...
var _checkChanelInterruption = function(
  data,
  oID,
  latestIncommingData,
  latestInteruptedChanels
  ...
  var result = {
    createAlert: false,
    save: false,
    latestIncommingData: latestIncommingData,
    latestInteruptedChanels: latestInteruptedChanels
  };

  if (latestInteruptedChanels[oID] && data) {
    //is always set by default
    if (!latestInteruptedChanels[oID][data.tank]) {
      //make sure the interuption has not been registered yet
      if (latestIncommingData[oID][data.tank]) {
        //if not the first time to get this data
        if (
          latestIncommingData[oID][data.tank].cdct +
            latestIncommingData[oID][data.tank].cdcns ===
          data.cdct + data.cdcns
        ) {
          //if same as previous value
          if (
            Math.abs(new Date(data.dct) - new Date(data.cdct)) >
            plcConfig.sTimer - 10000
          ) {
            //check difference betwen sample and transmition times.
  ...
  if (m && m.pl && m.pl.user && m.pl.user.oID) {
    if (m.pl && m.pl.tank) {
      Tank.findOneAndUpdate(
        { _id: m.pl.tank._id, oID: m.pl.user.oID },
        m.pl.tank,
        { new: true },
        function(err, tank) {
          if (err) {
