...
@Component({
  ...
  templateUrl:
    config.prefix + "/components/gps/map/shipment-map.component.html",
...
export class ShipmentMap implements AfterViewInit, OnDestroy {
    ...
    this.request.get("/plc/latest/withaddress.json").subscribe(resp => {
      ...
      if (resp && resp.pl && resp.pl.plc && resp.pl.address) {
        ...
        this.request.get("/users/offline.json").subscribe(res => {
          ...
          this.request.get("/gps/sim/all.json").subscribe(resp => {
  ...
  updateShowAllPosition(cardata) {
    if (ShipmentMap.allMarkers[cardata.sim]) {
      ...
      ShipmentMap.allMarkers[cardata.sim].setPosition(
        new BMap.Point(cardata.lng, cardata.lat)
      );
    }
  }
  ...
  showShipmentRoute(car, dest) {
    ...
    var route = new BMap.DrivingRoute(ShipmentMap.gpsmap, {
      renderOptions: { map: ShipmentMap.gpsmap, autoViewport: true }
    }); //驾车实例
    ...
    route.setSearchCompleteCallback(function() {
      ...
      that.newShipment.oti = this.route.queryParams.map(
        params => params["tank"] || "None"
      );
  ...
  showCar(sim: string) {
    ...
    if (this.targetCar) {
      var point = new BMap.Point(
        this.targetCar.lng.toFixed(3),
        this.targetCar.lat.toFixed(3)
      );
