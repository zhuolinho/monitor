...
lib.procesDownloadStats = function(param){

  var deferred = q.defer();
  const data = [["罐号", "日期", "累计流量","用量"]];
  for (var i = 0; i < param.length; i++) {
              data.push(['L001',param[i].date,param[i].maxVal,param[i].usage]);
  }
...
lib.procesDownloadInstantPlc = function(param){

  var deferred = q.defer();
  const data = [["罐号", "时间", "瞬时流量"]];
  for (var i = 0; i < param.length; i++) {
              data.push(['L001',param[i].cd,param[i].isc2]);
  }
